...
pub fn parse(source: &str) -> std::result::Result<Module, pest::error::Error<Rule>> {
    let mut ast = Module { path: String::new(), public: false};
...
mod parser_tests {
    ...
    #[test]
    fn module_type_type_decl() {
        assert!(parse("type Int;").is_ok());
    }

    #[test]
    fn module_type_object() {
        assert!(parse("type allias Object = {du:Int,ba:Int};").is_ok());
    }
